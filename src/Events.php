...
class Events
{
     ...
     */
    public static function GetAll(DateTimeInterface $start, DateInterval $length): array {
        $start = Cache::RoundDate($start);
        $key = self::GetCacheKey($start, $length);
        return Cache::GetWithLock($key, Cache::DefaultExpiry($start), function () use ($start, $length) {
            return Events::Get($start, $length, Sources::All());
        });
    }
     ...
     */
    public static function GetFromSource(array $sources, DateTimeInterface $start, DateInterval $length): array {
        if (count($sources) == 0) {
            return [];
        }
        $start = Cache::RoundDate($start);
        ...
        return Cache::GetWithLock($key, Cache::DefaultExpiry($start), function () use ($sources, $start, $length) {
            return Events::Get($start, $length, $sources);
        });
    }
     ...
     */
    private static function Get(DateTimeInterface $start, DateInterval $length, array $sources): array {
        $requests = Events::SendAsyncQueries($start, $length, $sources);
     ...
     */
    private static function SendAsyncQueries(DateTimeInterface $start, DateInterval $length, array $sources): array {
        $promises = [];
